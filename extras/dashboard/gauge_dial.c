...
{
  int16_t tipR = d->radius - d->arcWidth - 18;
  ...
  if (value != oldValue && oldValue != 0xFFFFFFFF)
  {
    ...
    DrawThickLine(tlx, tly, tx, ty, bgColor, bgColor, width);
  }
  ...
  uint16_t shadowColor = RGB565(r * 1 / 4, g * 1 / 4, b * 1 / 4);

  DrawThickLine(tlx + 1, tly + 1, tx + 1, ty + 1, shadowColor, shadowColor, width);
  DrawThickLine(tlx, tly, tx, ty, color, edgeColor, width);
}

void DIAL_DrawNeedle(GaugeDial *d, uint32_t value, uint16_t color, uint16_t bgColor, uint32_t oldValue)
{
  DIAL_DrawNeedleWithWidth(d, value, color, bgColor, oldValue, 9);
}
...
{
  DIAL_DrawCircle(d->cx, d->cy, d->radius + 3, shadowColor);
  DIAL_DrawCircle(d->cx, d->cy, d->radius + 2, innerColor);
  DIAL_DrawCircle(d->cx, d->cy, d->radius + 1, outerColor);
}
